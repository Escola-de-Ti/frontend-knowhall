...
export default function PerfilDetalhes({
  ...
  const initials = nome?.trim()?.split(' ').map(p => p[0]).slice(0,2).join('') || 'U'

  return (
    <section className="perfil-details-container" data-userid={id_usuario} data-status={status_usuario} data-tipo={tipo_usuario}>
      <header className="perfil-head">
        <div className="perfil-avatar">
          {id_imagem_perfil ? (
            <img src={id_imagem_perfil} alt={nome} />
          ) : (
            <span>{initials}</span>
          )}
        </div>
      ...
      <div className="perfil-actions">
        <button className="perfil-edit-btn" onClick={onEditar}>Editar Perfil</button>
      </div>
